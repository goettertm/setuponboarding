      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between mb-8">
          {steps.map((step, index) => <div key={step.number} className="flex flex-col items-center">
              <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 mb-2 transition-colors ${step.completed ? 'bg-green-500 border-green-500 text-white' : currentStep === step.number ? 'border-blue-500 text-blue-500 bg-white' : 'border-gray-300 text-gray-400 bg-white'}`}>
                {step.completed ? <CheckCircle className="w-6 h-6" /> : <span className="text-sm font-medium">{step.number}</span>}
              </div>
              <span className={`text-xs text-center ${currentStep === step.number ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>
                {step.title}
              </span>
            </div>)}
        </div>
